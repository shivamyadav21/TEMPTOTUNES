...
const playSong = () => {
  ...
  audioelement.play().then(() => {
    myprogressbar.setAttribute('value', 0);
    myprogressbar.setAttribute('max', audioelement.duration);
    const min = Math.round((audioelement.duration)/60);
    const sec = Math.abs(Math.round(audioelement.duration - Math.round((audioelement.duration)/60)*60));
    document.getElementById(songindex).parentElement.getElementsByClassName('t')[0]
    .innerHTML = min + ':' + ((sec < 10)? '0'+sec : sec);
    gif.style.opacity=1;
  });
